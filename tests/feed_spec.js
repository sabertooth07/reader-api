...
function subFirstUserToDilbertFeed(callback) {
    var user = TEST_USERS[0];
    frisby.create("Subscribe first user to the Dilbert feed using PUT " + user.email)
...
function subFirstUserToDilbertFeedAgain(callback) {
    var user = TEST_USERS[0];
    frisby.create("Subscribe first user to the Dilbert feed using PUT " + user.email)
            .put(tc.url + "/feeds/subscribe", { feedURL: dilbertFeedURL})
            .auth(user.sp_api_key_id, user.sp_api_key_secret)
...
function subFirstUserToNycEaterFeed(callback) {
    var user = TEST_USERS[0];
    frisby.create("Subscribe first user to the Dilbert feed using PUT " + user.email)
...
function subSecondUserToDilbertFeed(callback) {
